...
def check_installed_modules():
    ...
    cursor.execute(
        ...
        WHERE name LIKE '%freezon%' OR name = 'hr_employee_custom'
    """
    )
...
def get_views():
    """Get all views created by the hr_employee_custom module using
    ir_model_data"""
    conn = psycopg2.connect(
        dbname=DB_NAME, user=DB_USER, password=DB_PASSWORD, host=DB_HOST, port=DB_PORT
    )
...
1. ✅ VIEW REFERENCE TEST
   └── self.env.ref('hr_employee_custom.view_name')
...
The hr_employee_custom module now has comprehensive test coverage for ALL views.

...
from odoo.http import _logger, request, route
...
class KsCurvedBackendTheme(http.Controller):
    ...
    def ks_save_form_page_width(self, val):
        request.env.user.write({"ks_form_page_width": val})

    @route(
        ["/ks_curved_backend_theme_enter/save/ks_splitted_form_width"],
        type="json",
        auth="user",
    )
    ...
    def update_bookmarks(self, **post):
        ...
        if post.get("create_new", False):
            ...
            request._cr.execute(
                "update ks_bookmark set ks_bookmark_name=%s where id = %s and ks_user_id = %s ",
            ...
            if new_position:
                request._cr.execute(
                    "update ks_bookmark set ks_bookmark_position=%s where id = %s and ks_user_id = %s ",
            ...
            bookmark_id = post["bookmark_id"]
            request._cr.execute(
                "Delete from ks_bookmark where id =%s and ks_user_id = %s",
                (bookmark_id, user_id),
            )
        ks_bookmarks = request.env["ks.bookmark"].search(
            [("ks_user_id", "=", user_id)], order="ks_bookmark_position"
        )
        values = {
            "bookmarks": ks_bookmarks,
        }
        return request.env["ir.ui.view"]._render_template(
            "ks_curved_backend_theme_enter.ks_bookmark_", values
        )
    ...
    def render_theme_view_settings(self, ks_setting_scope, ks_rec_id=None):
        ...
        if ks_setting_scope == "User":
            ...
            obj_list = {
                "global": request.env.ref(
                    "ks_curved_backend_theme_enter.ks_global_config_single_rec"
                ),
            ...
            if ks_all_setting_scope["scope_ks_colors_theme"] == "User":
                ks_color_theme = request.env(su=True)["ks.color.theme"].get_theme(
                    scope=ks_setting_scope, origin=user
                )
            values.update(
                {
                    ...
                    "ks_manager_role": user.has_group(
                        "ks_curved_backend_theme_enter.ks_curved_theme_settings"
                    ),
                    "ks_global_config_id": request.env.ref(
                        "ks_curved_backend_theme_enter.ks_global_config_single_rec"
                    ).id,
                    "ks_body_background_img": self.ks_get_background_data(
                        ks_domain, "ks.body.background"
                    ),
                    "ks_app_drawer_background_img": self.ks_get_background_data(
                        ks_domain, "ks.drawer.background"
                    ),
                    "ks_body_background_opacity": obj_list[
                        ks_all_setting_scope.get("scope_ks_body_background").lower()
                    ...
                    "ks_body_background_image_enable": obj_list[
                        ks_all_setting_scope.get("scope_ks_body_background").lower()
                    ].ks_body_background_image_enable,
            ...
            if ks_current_menu_bar_scope == "Global":
                ks_origin_data = request.env.ref(
                    "ks_curved_backend_theme_enter.ks_global_config_single_rec"
                )
                ks_new_values = self.ks_get_values(
                    {"scope_ks_menu_bar": ks_current_menu_bar_scope},
                    ks_origin_data,
        ...
        if ks_setting_scope == "Company":
            ...
            values = self.ks_get_values(
                ks_all_setting_scope, ks_origin_data, ks_setting_scope
            )
        ...
        if ks_setting_scope in ["Company", "Global"]:
            ks_color_theme = False
            if (
                ks_all_setting_scope["scope_ks_colors_theme"] == "Company"
                and ks_setting_scope == "Company"
            ) or (ks_setting_scope == "Global"):
                ks_color_theme = request.env(su=True)["ks.color.theme"].get_theme(
                    scope=ks_setting_scope, origin=ks_origin_data
                )
            values.update(
                {
                    ...
                    "ks_small_company_logo": self.ks_get_image_url(
                        ks_origin_data.ks_small_company_logo
                    ),
                    "ks_body_background_img": self.ks_get_background_data(
                        ks_domain, "ks.body.background"
                    ),
            ...
            if "ks_company_logo" in values:
                values.update(
                    {
                        "ks_company_logo": self.ks_get_image_url(
                            ks_origin_data.ks_company_logo
                        )
                    }
                )
        ...
        if ks_setting_scope == "Global":
            ...
            values["ks_login_page_logo_enable"] = (
                ks_origin_data.ks_login_page_logo_enable
            )
            values["ks_login_page_logo_image"] = self.ks_get_image_url(
                ks_origin_data.ks_login_page_logo
            )
            values["ks_color_theme_scope"] = ks_all_setting_scope[
                "scope_ks_colors_theme"
            ]
            values["ks_login_page_style"] = ks_origin_data.ks_login_page_style
            values["ks_login_background_color"] = request.env[
                "ks.login.background.color"
            ].search_read([])
    ...
    def ks_get_image_url(self, data):
        ...
        return (
            "data:image/"
            + (file_type_magic_word.get(data[0]) or "png")
            + ";base64,"
            + data.decode("utf-8")
        )
    ...
    def ks_get_values(self, ks_all_setting_scope, ks_origin, ks_current_scope):
        ...
        for key, value in ks_all_setting_scope.items():
            if value == ks_current_scope and key.split("scope_")[1] in ks_origin:
                values[key.split("scope_")[1]] = (
                    ks_origin[key.split("scope_")[1]]
                    if ks_origin[key.split("scope_")[1]]
                    else True
                )
    ...
    def ks_get_background_data(self, ks_domain, ks_model):
        ...
        for ks_background in request.env[ks_model].search(ks_domain):
    ...
    def ks_save_split_data(self, val):
        request.env.user.write({"ks_split_view": val})
        return True
    ...
    @route(
        ["/ks_curved_backend_theme_enter/add/login/images"], type="json", auth="user"
    )
    def ks_add_login_background_images(self, image_info=False):
        # Save data.
        if image_info.get("key") and image_info.get("value"):
            request.env["ks.login.background.image"].create(
                {"ks_image": image_info["value"]}
            )
    ...
    def save_theme_settings(self, **post):
        ...
        ks_login_page_fields = [
            "ks_login_background_image_save",
            "ks_login_background_setting",
        ]
        ks_theme_fields = [
            "ks_body_background",
            "ks_menu",
        ...
        if post.get("ks_origin_scope") == "user":
            ...
            ks_color_theme_domain = [
                ("ks_user", "=", ks_origin_obj.id),
                ("ks_active", "=", True),
            ]
        ...
        if post.get("ks_origin_scope") == "company":
            ...
            ks_color_theme_domain = [
                ("ks_company", "=", ks_origin_obj.id),
                ("ks_active", "=", True),
            ]

        if post.get("ks_origin_scope") == "global":
            ks_origin_obj = request.env.ref(
                "ks_curved_backend_theme_enter.ks_global_config_single_rec"
            )
            ks_splitter = "_global"
        ...
        for key, value in post.get("ks_unsaved_setting").items():
            ...
            if key in ks_no_check_field:
                if type(value) is int:
                    ...
                    for ks_background in request.env[ks_image_model[key]].search(
                        ks_background_domain
                    ):
                ...
                request.env["ks.login.background.image"].search([]).write(
                    {"ks_active": False}
                )
                request.env["ks.login.background.color"].search([]).write(
                    {"ks_active": False}
                )
                # Active login page background based on image or color.
                if key == "ks_login_background_image_save":
                    ks_rec = request.env["ks.login.background.image"].search(
                        [("id", "=", value)]
                    )
                    if ks_rec:
                ...
                if key == "ks_login_background_setting" and value:
                    ks_rec = request.env["ks.login.background.color"].search(
                        [("id", "=", value)]
                    )
                    if ks_rec:
                        ks_rec.ks_active = True
                ...
                if type(value) == str and (value.upper() == value.lower()):
                    intnum = float(value)
                    if value == str(intnum):
                        value = float(value)
                    else:
            ...
            if key == "ks_theme_color":
                ...
                ks_current_theme = (
                    request.env["ks.color.theme"].sudo().search(ks_color_theme_domain)
                )
                for rec in ks_current_theme:
                    rec.write({"ks_active": False})
                # active new theme
                ks_new_theme = (
                    request.env["ks.color.theme"]
                    .sudo()
                    .search([("id", "=", value)], limit=1)
                )
                if ks_new_theme:
        ...
        if "ks_enterprise_apps" in post["ks_unsaved_setting"]:
            request.env.ref(
                "ks_curved_backend_theme_enter.ks_curved_theme_hide_enterprise_apps"
            ).write({"active": post["ks_unsaved_setting"].get("ks_enterprise_apps")})
    ...
    def ks_list_render(self, **kw):
        ...
        try:
            attachment_ids = (
                ...
                .search(
                    [
                        ("res_model", "=", tree_view_model),
                        ("res_id", "in", tree_view_ids),
                    ]
                )
                .ids
            )
    ...
    def ks_app_frequency_update(self, **kw):
        ...
        if menu_id in user_id.ks_frequency_menu.ks_menu_id.ids:
            menu = user_id.ks_frequency_menu.search(
                [("ks_menu_id", "=", menu_id), ("ks_user_id", "=", user_id.id)], limit=1
            )
    ...
    def set_background_default(
        self, ks_setting_scope, ks_rec_id=None, ks_default_info=None
    ):
        if ks_setting_scope == "Global" and ks_default_info:
            ks_global_obj = request.env.ref(
                "ks_curved_backend_theme_enter.ks_global_config_single_rec"
            )
        ...
        if ks_setting_scope == "Company" and ks_rec_id and ks_default_info:
            ...
            # de-active background image.
            ks_sel_background = request.env[ks_default_info["model"]].search(
                [("ks_company", "=", ks_company_obj.id), ("ks_active", "=", True)],
                limit=1,
            )
            if ks_sel_background:
                ks_sel_background.ks_active = False
    ...
    def ks_get_theme(self):
        ks_color_theme_scope = request.env["ks.global.config"].ks_get_field_scope(
            ["scope_ks_colors_theme"]
        )
        ...
        ks_current_theme_data = (
            request.env["ks.color.theme"].sudo().search(ks_color_theme_domain, limit=1)
        )
...
class KsHome(Home):
    ...
    def ks_get_login_page_image(self):
        ...
        ks_check_active_image = (
            request.env["ks.login.background.image"]
            .sudo()
            .search([("ks_active", "=", True)], limit=1)
        )
        ...
        ks_check_active_color = (
            request.env["ks.login.background.color"]
            .sudo()
            .search([("ks_active", "=", True)], limit=1)
        )
        if ks_check_active_color:
            return {"type": "color", "background-color": ks_check_active_color.ks_color}
